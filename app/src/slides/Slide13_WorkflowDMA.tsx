...
export default function Slide13_WorkflowDMA() {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ background: '#141416' }}>
      <motion.div className="absolute inset-0" style={{ backgroundImage: 'url(/images/workflow.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.14 }}
        initial={{ scale: 1.06, opacity: 0 }} animate={{ scale: 1, opacity: 0.14 }} transition={{ duration: 2.5, ease: [0.22, 1, 0.36, 1] }} />
      <div className="absolute inset-0" style={{ background: 'linear-gradient(180deg, rgba(0,0,0,0.35) 0%, rgba(0,0,0,0.15) 30%, rgba(0,0,0,0.15) 65%, rgba(0,0,0,0.4) 100%)' }} />
      ...
      <div className="absolute inset-0 flex flex-col" style={{ paddingLeft: '6cqw', paddingRight: '5cqw', paddingTop: '4cqh', paddingBottom: '3.5cqh' }}>
        ...
        <div style={{ marginBottom: '3.5cqh' }}>
          <div className="overflow-hidden">
            <motion.h1 initial={{ y: '105%' }} animate={{ y: 0 }} transition={{ duration: 0.8, delay: 0.4, ease: [0.22, 1, 0.36, 1] }}
              style={{ fontSize: '5.5cqh', fontWeight: 800, lineHeight: 1.1, color: '#fff', letterSpacing: '-0.025em', margin: 0 }}>
              {t.title}{' '}
            ...
            <div key={step.num} className="flex items-center" style={{ flex: 1, gap: '0.5cqw' }}>
              <motion.div className="flex flex-col items-center justify-center"
                style={{ flex: 1, padding: '2cqh 0', borderRadius: '1cqh',
                  background: i === 3 ? 'rgba(200,16,46,0.12)' : 'var(--surface-card)',
        ...
        <div className="flex-1 grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.2cqw' }}>
          {t.roles.map((r, i) => {
            const Icon = ICONS[i]
            return (
              <motion.div key={i} style={{ padding: '2.5cqh 1.8cqw', borderRadius: '1cqh', background: 'var(--surface-card)',
                ...
                <Icon style={{ width: '4cqh', height: '4cqh', color: '#c8102e' }} strokeWidth={1.5} />
                <div>
                  <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '2.0cqh', fontWeight: 700, color: '#c8102e', margin: 0 }}>{r.role}</p>
                  <p style={{ fontSize: '2.0cqh', color: 'var(--text-muted)', margin: 0, marginTop: '0.6cqh', lineHeight: 1.4 }}>{r.action}</p>
                </div>
              </motion.div>
            )
          })}
